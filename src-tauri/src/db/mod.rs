...
CREATE INDEX IF NOT EXISTS idx_media_event_group ON media_items(event_group_id);
    ...
    ensure_column(&conn, "event_groups", "ai_prompt_version", "TEXT")?;
    ensure_column(&conn, "event_groups", "ai_model_used", "TEXT")?;
    ensure_column(&conn, "event_groups", "ai_location_used", "TEXT")?;
    ensure_column(&conn, "event_groups", "ai_needs_fallback", "INTEGER DEFAULT 0")?;
    ensure_column(&conn, "event_groups", "ai_fallback_used", "INTEGER DEFAULT 0")?;
